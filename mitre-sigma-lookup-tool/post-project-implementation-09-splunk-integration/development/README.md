...
- Furthermore, if I scroll down, you can also see the recent detection activity filled with the `time` and `savedsearch_name` columns. Lastly, you have the trigger count and this is only 1 because I didn't run anything after (My alert suppress period was for 30 minutes so it did not fire again at the time of this writing), the 7 day trend and the top rule which is `T1059.003`
